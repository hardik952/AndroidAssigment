...
class ToDoAdapter(var context: Context, var taskList:MutableList<Todo>):
    RecyclerView.Adapter<ToDoAdapter.TodoMyViewHolder>() {

    var itemEditClickListener:((position:Int,task:Todo)->Unit)?=null
    var itemDeleteClickListener:((position:Int,task:Todo)->Unit)?=null
    class TodoMyViewHolder(var binding: LayoutTodoItemBinding): RecyclerView.ViewHolder(binding.root)
